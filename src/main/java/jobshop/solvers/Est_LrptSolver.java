...
public class Est_LrptSolver implements Solver {
    ...
    public Result solve(Instance instance, long deadline) {
        int duree[] = new int[instance.numJobs];
        ...
        while(nbTachesTotal >0){
            ...
            Task aux = Lrpt(instance,realisables,tachesCourant,duree,machine);
    ...
    public static Task Lrpt(Instance instance, ArrayList<Task> taches, int[] tacheC, int[] duree, int[] machine){
        //prio au job ayant la duréée restante la plus courte
        int[] dureeRestante = new int[instance.numJobs];
        int aux, indiceJob;

        Task auxT = taches.get(0);
